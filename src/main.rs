...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let prompt = format!("Analyze these test results and provide insights:\n\n{}", sample_results);
    let response = model.generate(&prompt).await?;
    ...
    let prompt = format!("Based on these test results, suggest improvements to the tests or the system under test:\n\n{}", sample_results);
    let response = model.generate(&prompt).await?;
