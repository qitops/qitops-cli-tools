...
impl DataDrivenRunner {
    ...
    fn load_csv_data(data_source: &DataSource) -> Result<Vec<HashMap<String, String>>> {
        ...
        let file = File::open(file_path)
            .map_err(|e| Error::IoError(format!("Failed to open CSV file: {}", e)))?;
        ...
        // Reset the reader if we used the first record to generate headers
        let mut csv_reader = if !data_source.has_header {
    ...
    fn load_json_data(data_source: &DataSource) -> Result<Vec<HashMap<String, String>>> {
        ...
        let file = File::open(file_path)
            .map_err(|e| Error::IoError(format!("Failed to open JSON file: {}", e)))?;
        ...
        let data = if data_source.json_path == "$" {
            ...
            let selector = jsonpath_lib::selector(json_value, json_path)
