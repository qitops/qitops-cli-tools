...
use data_driven::{DataDrivenConfig, DataDrivenRunner};
use error::Result;
use performance::{PerformanceTestConfig, PerformanceTestRunner};
use performance_enhanced::{EnhancedPerformanceConfig, EnhancedPerformanceRunner};
...
async fn main() -> Result<()> {
    ...
    let result = match &cli.command {
        ...
        Commands::DataDriven { config, data, data_type, environment, max_iterations, stop_on_failure } => {
            ...
            let test_type = schema::determine_test_type(&config)?;
            ...
            let data_source_type = match data_type.to_lowercase().as_str() {
                "csv" => data_driven::DataSourceType::Csv,
                "json" => data_driven::DataSourceType::Json,
                _ => return Err(Error::ValidationError(format!("Unsupported data source type: {}", data_type))),
            };

            let data_source = data_driven::DataSource {
            ...
            for (i, row) in runner.data_rows().iter().take(max_iter).enumerate() {
                ...
                let result = match test_type.as_str() {
                    ...
                    "collection" => {
                        ...
                        DataDrivenRunner::replace_placeholders(&mut collection_json_mut, row);
                        let updated_collection: api_collection::ApiCollection = serde_json::from_value(collection_json_mut)?;
                        ...
                        Ok(collection_result.request_results.first().cloned().unwrap_or_default())
                    },
