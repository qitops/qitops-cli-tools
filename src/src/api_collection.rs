...
impl ApiCollectionRunner {
    ...
    async fn execute_request(&self, request: &CollectionRequest, collection: &ApiCollection, variables: &HashMap<String, String>) -> Result<TestResult> {
        ...
        if let Some(validation_issues) = validation_result {
            details["validation_issues"] = json!(validation_issues);
        }

        // Determine test status
        let test_status = if status.is_success() && validation_result.is_none() {
            "passed"
        } else {
            "failed"
        };
