...
# Test script for QitOps AI features with Ollama

echo "QitOps AI Features Test with Ollama"
...
# Make sure we're in the main QitOps directory
cd "$(dirname "$0")"
cargo build --features ai
if [ $? -ne 0 ]; then
    echo "Error: Failed to build QitOps with AI features"
    exit 1
fi
...
mkdir -p ollama_test_outputs
cd ollama_test_outputs
